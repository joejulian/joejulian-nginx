...
#   [*ssl_client_certificate*] - This directive specifies the file containing the CA (root) certificate, in PEM format, that is used for validating client certificates.
...
define nginx::resource::vhost(
  ...
  if ($listen_port != $ssl_port) {
    file { "${nginx::config::nx_temp_dir}/nginx.d/${stubname}-001":
      ensure  => $ensure ? {
        'absent' => absent,
        default  => 'file',
      },
  ...
  nginx::resource::location {"${stubname}-default":
    ensure               => $ensure,
    vhost                => $stubname,
    ssl                  => $ssl,
    ssl_only             => $ssl_only,
  ...
  if ($listen_port != $ssl_port) {
    file { "${nginx::config::nx_temp_dir}/nginx.d/${stubname}-699":
      ensure  => $ensure ? {
        'absent' => absent,
        default  => 'file',
      },
  ...
  if ($ssl == 'true') {
    file { "${nginx::config::nx_temp_dir}/nginx.d/${stubname}-700-ssl":
      ensure => $ensure ? {
        'absent' => absent,
        default  => 'file',
      },
    ...
    file { "${nginx::config::nx_temp_dir}/nginx.d/${stubname}-999-ssl":
      ensure => $ensure ? {
        'absent' => absent,
        default  => 'file',
      },
